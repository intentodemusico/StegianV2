...
aux = EmbeddingSimulator(cover, rhoP1, rhoM1, round(numel(cover)*payload), false);
coverOriginal(:,:,channel)=aux;
stego=uint8(coverOriginal);

% compute distortion
distM1 = rhoM1(double(aux)-double(cover)==-1);
distP1 = rhoP1(double(aux)-double(cover)==1);
